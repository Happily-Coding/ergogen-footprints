...
//
// Description:
//    Virtual footprint for keycap BOM/POS/3D preview only.
...
//    to track keycaps separately from the switch footprint.
//
// Nets:
...
//      Path to the 3D model STEP or WRL file for the keycap
...
module.exports = {
  ...
  body: p => {
    const footprint = `
  (footprint "ceoloide:keycap-fab-preview_choc_v1_v2"
    (layer "${p.side}.Cu")
    ${p.at}
    ...
    ${p.supplier_link ? `    (property "Keycap Supplier Link" "${p.supplier_link}")` : ''}
    ${p.manufacturer_part_number ? `    (property "Keycap manufacturer_part_number" "${p.manufacturer_part_number}")` : ''}
    ${p.do_not_populate ? `    (property "Keycap Do Not Populate" "${p.do_not_populate}")` : ''}
    ${p.model_filename ? `
